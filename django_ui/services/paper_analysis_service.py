...
class PaperAnalysisService:
    ...
    def __init__(self):
        # Initialize Llama 3.2 3B text processor
        from .llama_text_processor import llama_text_processor
        self.text_processor = llama_text_processor
        
        # Initialize SpaCy for additional NLP features (optional)
        try:
            import spacy
            self.nlp = spacy.load("en_core_web_sm")
        except (OSError, ImportError):
            logger.warning("SpaCy model not found. Using Llama 3.2 3B for all NLP tasks")
            self.nlp = None
    ...
    async def generate_summary(self, paper_data: Dict) -> str:
        """Generate a concise summary of the paper using Llama 3.2 3B"""
        try:
            ...
            # Use Llama 3.2 3B for summarization
            summary = self.text_processor.summarize_text(abstract, max_length=300)
    ...
    async def extract_keywords(self, paper_data: Dict) -> List[str]:
        """Extract important keywords from the paper using Llama 3.2 3B"""
        try:
            ...
            # Extract keywords using Llama 3.2 3B
            keywords = self.text_processor.extract_keywords(text, max_keywords=15)
    ...
    async def classify_research_fields(self, paper_data: Dict) -> List[Dict]:
        """Classify the paper into research fields using Llama 3.2 3B"""
        try:
            text = f"{paper_data.get('title', '')} {paper_data.get('abstract', '')}"
            keywords = await self.extract_keywords(paper_data)
            ...
            classification_scores = self.text_processor.classify_text(text, research_fields)
            
            # Convert to required format
            field_scores = []
            for field, confidence in classification_scores.items():
                if confidence > 0.1:  # Only include fields with meaningful confidence
                    field_scores.append({
                        "field": field,
                        "confidence": confidence,
                        "matched_keywords": [kw for kw in keywords if kw.lower() in text.lower()][:5]
                    })
